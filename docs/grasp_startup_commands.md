...
| 话题 | 消息类型 | 方向 | 用途 |
| --- | --- | --- | --- |
| `/camera/color/image_raw` | `sensor_msgs/msg/Image` | 发布 | Orbbec 相机发布彩色图像，检测节点订阅后进行小黄鸭、绿色药盒、红色圆或 YOLO 物体识别 |
...
| `/detect_yolo/apple_position` | `geometry_msgs/msg/PointStamped` | 发布 | YOLO 节点发布苹果在相机坐标系下的三维位置 |
| `/detect_yolo/banana_position` | `geometry_msgs/msg/PointStamped` | 发布 | YOLO 节点发布香蕉在相机坐标系下的三维位置 |
| `/detect_yolo/bottle_position` | `geometry_msgs/msg/PointStamped` | 发布 | YOLO 节点发布瓶子在相机坐标系下的三维位置 |
| `/detect_yolo/cake_position` | `geometry_msgs/msg/PointStamped` | 发布 | YOLO 节点发布蛋糕在相机坐标系下的三维位置 |
| `/red_circle_position` | `geometry_msgs/msg/PointStamped` | 发布 | 传统颜色 detector 发布红色圆在相机坐标系下的三维位置 |
...
| `detect_yolo_node` | `/camera/color/image_raw`、`/camera/depth/image_raw`、`/camera/color/camera_info` | `/detect_yolo/apple_position`、`/detect_yolo/banana_position`、`/detect_yolo/bottle_position`、`/detect_yolo/cake_position` 等 | YOLO 识别物体并输出相机坐标系三维点 |
| `red_circle_detector_node` | `/camera/color/image_raw`、`/camera/depth/image_raw`、`/camera/color/camera_info` | `/red_circle_position` | 传统颜色阈值检测红色圆并输出相机坐标系三维点 |
...
| `camera_to_base_transform.py` | `/robot_arm/end_pose`、`/detect_yolo/apple_position`、`/detect_yolo/banana_position`、`/detect_yolo/bottle_position`、`/detect_yolo/cake_position`、`/red_circle_position`、`/duck_position`、`/box_position` | `/visual_target_base` | 将相机坐标系目标点转换到 `base_link` 坐标系 |
...
| 2 | `/camera/color/image_raw`、`/camera/depth/image_raw`、`/camera/color/camera_info` | 检测节点 | `/detect_yolo/apple_position`、`/detect_yolo/banana_position`、`/detect_yolo/bottle_position`、`/detect_yolo/cake_position`、`/red_circle_position`、`/duck_position` 或 `/box_position` | 目标在相机坐标系下的三维坐标 |
...
| 项目 | 说明 |
| --- | --- |
| 坐标系 | `/detect_yolo/apple_position`、`/detect_yolo/banana_position`、`/detect_yolo/bottle_position`、`/detect_yolo/cake_position`、`/red_circle_position`、`/duck_position`、`/box_position` 是相机坐标系；`/visual_target_base` 是 `base_link` 坐标系 |
| 检测节点选择 | 苹果、香蕉、瓶子、蛋糕优先使用常驻 YOLO；传统 detector 保留小黄鸭、绿色药盒和大樱桃（红色圆） |
| YOLO 启动策略 | 推荐上电后常驻运行 `detect_yolo_node`，这样语音问“桌子上有什么”时可以直接读取 `/vision/scene_text`；`arm_task_manager.py` 默认复用常驻 YOLO，不重复拉起 YOLO 进程 |
| 抓取目标过滤 | `arm_task_manager.py` 会发布 `/arm_task/active_object`，管理节点启动的 `camera_to_base_transform.py` 只转发当前语音指定目标，避免常驻 YOLO 多物体坐标互相干扰 |
